...
const showAddStopBtn = document.getElementById('showAddAddStopBtn');
...
async function showSection(sectionId) {
    ...
    showStatusMessage('', 'success', statusMessageGlobal);
}
...
const addStopStatus = document.getElementById('addStopStatus');
const allStopsList = document.getElementById('allStopsList');
const cancelEditStopButton = document.getElementById('cancelEditStopButton');
async function handleAddEditStop() {
    ...
    if (!name) {
        showStatusMessage('กรุณากรอกข้อมูลชื่อจุดจอด', 'error', addStopStatus);
        return;
    }
    try {
        if (editingStopId) {
            await updateDoc(doc(db, 'stops', editingStopId), {
                name: name,
                latitude: latitude,
                longitude: longitude
            });
            showStatusMessage(`แก้ไขจุดจอด "${name}" (ID: ${editingStopId}) สำเร็จ!`, 'success', addStopStatus);
        } else {
            const newStopId = await generateNextId('stops', lastStopId);
            await setDoc(doc(db, 'stops', newStopId), {
                name: name,
                latitude: latitude,
                longitude: longitude
            });
            showStatusMessage(`เพิ่มจุดจอด "${name}" (ID: ${newStopId}) สำเร็จ!`, 'success', addStopStatus);
            lastStopId = parseInt(newStopId, 10);
        }
        clearAddStopForm();
        await loadAllStopsForList();
        await loadStopsIntoDropdowns();
        ...
        console.error("ข้อผิดพลาดในการบันทึกจุดจอด:", error);
        showStatusMessage(`เกิดข้อผิดพลาดในการบันทึกจุดจอด: ${error.message}`, 'error', addStopStatus);
    }
}
async function loadAllStopsForList() {
    ...
    try {
        ...
        allStops.sort((a, b) => a.name.localeCompare(b.name, 'th'));
        allStops.forEach(stop => {
            ...
            stopItemDiv.innerHTML = `
                <div class="stop-item-info">
                    ...
                    Lat: ${stop.latitude || 'N/A'}, Lon: ${stop.longitude || 'N/A'}
                </div>
                <div class="stop-item-actions">
                    <button class="secondary edit-stop-btn" data-id="${stop.id}">แก้ไข</button>
                    <button class="danger delete-stop-btn" data-id="${stop.id}">ลบ</button>
                </div>
            `;
            allStopsList.appendChild(stopItemDiv);
        });
        allStopsList.querySelectorAll('.edit-stop-btn').forEach(button => {
            button.addEventListener('click', (event) => editStop(event.target.dataset.id));
        });
...
function editStop(stopId) {
    const stopToEdit = allStops.find(s => s.id === stopId);
    if (!stopToEdit) {
        showStatusMessage('ไม่พบจุดจอดที่ต้องการแก้ไข', 'error', addStopStatus);
        return;
    }
    editingStopId = stopId;
    stopNameInput.value = stopToEdit.name || '';
    stopLatInput.value = stopToEdit.latitude || '';
    stopLonInput.value = stopToEdit.longitude || '';
    addStopButton.textContent = 'บันทึกการแก้ไข';
    cancelEditStopButton.style.display = 'inline-block';
    showStatusMessage('กำลังแก้ไขจุดจอด...', 'info', addStopStatus);
    addStopContainer.scrollIntoView({ behavior: 'smooth' });
}
...
function clearAddStopForm() {
    ...
    stopLatInput.value = '';
    stopLonInput.value = '';
    addStopButton.textContent = 'เพิ่มจุดจอด';
    cancelEditStopButton.style.display = 'none';
    if (addStopStatus) addStopStatus.textContent = '';
}
addStopButton.addEventListener('click', handleAddEditStop);
cancelEditStopButton.addEventListener('click', clearAddStopForm);
...
const manageRouteStatus = document.getElementById('manageRouteStatus');
...
async function saveRoute() {
    ...
    const routeStops = [];
    let isValid = true;
    let stopOrderMap = new Map();
    let stopIdMap = new Map();
    ...
    try {
        ...
        if (editingRouteId) {
            ...
            lastRouteId = parseInt(newRouteId, 10);
        }
        ...
        for (let i = 0; i < routeStops.length; i++) {
            for (let j = i + 1; j < routeStops.length; j++) {
                ...
                lastSegmentId = parseInt(newSegmentId, 10);
            }
        }
        await batch.commit();
